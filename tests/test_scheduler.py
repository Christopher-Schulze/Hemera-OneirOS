...
def test_schedule_matches_plan_segments(config: Path) -> None:
    ...
    assert schedule.timeline.total_duration_seconds == pytest.approx(
        schedule.summary.total_duration_seconds
    )
    assert any(event.kind == "aggregator" for event in schedule.timeline.events)
...
def test_schedule_prover_stats_utilisation(config: Path) -> None:
    ...
    assert "timeline" in serialised
    analytics = schedule.summary.analytics
    assert analytics.peak_concurrency >= 1
    assert analytics.busy_time_seconds == pytest.approx(
        sum(segment.duration_seconds for segment in schedule.segments)
    )
    assert analytics.idle_time_seconds == pytest.approx(
        sum(stat.idle_time_seconds for stat in schedule.summary.prover_stats)
    )
    for stat in schedule.summary.prover_stats:
        assert stat.idle_time_seconds >= 0
        if stat.segments:
            assert stat.first_segment_start_seconds is not None
            assert stat.last_segment_end_seconds is not None
            assert stat.last_segment_end_seconds >= stat.first_segment_start_seconds
        else:
            assert stat.first_segment_start_seconds is None
            assert stat.last_segment_end_seconds is None
        span = stat.busy_time_seconds + stat.idle_time_seconds
        if schedule.summary.total_duration_seconds > 0:
            assert span == pytest.approx(schedule.summary.total_duration_seconds)
    timeline_events = schedule.timeline.events
    assert all(event.end_time_seconds >= event.start_time_seconds for event in timeline_events)
